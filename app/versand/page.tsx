...
type ShippingSection = {
  id: number;
  title: string;
  iconId: ShippingIconId;
  content: ShippingContentItem[];
};
...
// CONSTANTS
const ICON_MAP: Record<ShippingIconId, LucideIcon> = {
  globe: Globe,
  clock: Clock,
  alert: AlertCircle,
  shield: Shield,
...
function ContentItem({ item }: ContentItemProps) {
  return (
    <div>
      ...
      {item.text && (
        <p className="text-base md:text-lg text-lightGray leading-relaxed mb-3">
          {item.text}
        </p>
      )}
      {item.list && (
        <ul className="space-y-2 mt-2">
          {item.list.map((entry, idx) => (
            <li
              key={idx}
              className="flex items-start gap-3 text-base md:text-lg text-lightGray leading-relaxed"
            >
              <span
                aria-hidden="true"
                className="mt-2.5 w-1.5 h-1.5 rounded-full bg-primaryOrange flex-shrink-0"
              />
              <span>{entry}</span>
            </li>
          ))}
...
export default function VersandPage() {
  ...
  // CONSTANTS
  const sections = (t.raw("sections") as ShippingSection[]) ?? [];
  const company = t.raw("company") as ShippingCompany;
